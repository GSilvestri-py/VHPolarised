...
class Analyzer:
    ...
    def run_analyzer(self):
        ...
        for i in range(self.tree.GetEntries()):
            #if i % 100 == 0:
                #print(f"[Worker] {self.filename}: processed {i} events", flush=True)
...
def process_file(filename):
    ...
    global MODEL
    global DNN_MODEL
    analyzer = Analyzer(filename, MODEL, DNN_MODEL)
    analyzer.run_analyzer()
    return analyzer.counters, analyzer.total_hbb_events

# ------- ------- ------- ------- ------- ------- ------- ------- ------- ------- -------

def init_worker(model_path, configfile):
    global MODEL
...
def main():
    ...
    configfile.read("config_efficiency.cfg")

    bdt_model_path = configfile.get('input_files', 'bdt_model_path')
    ...
    with ProcessPoolExecutor(max_workers=n_blocks, 
                             mp_context=multiprocessing.get_context("spawn"),
                             initializer=init_worker,
                             initargs=(bdt_model_path, configfile,)
                             ) as pool:
        results = list(
                        tqdm(
    ...
    for counters, total_hbb in results:
        for key in global_counters:
            for i in range(3):
                global_counters[key][i] += counters[key][i]
        global_total_hbb += total_hbb


    results_data = [
        ["DeepFlavB"]   + global_counters["DeepFlavB"],
        ["DeepFlavCvL"] + global_counters["DeepFlavCvL"],
        ["DeepFlavCvB"] + global_counters["DeepFlavCvB"],
        ["JetPt"]       + global_counters["JetPt"],
        ["BDT"]         + global_counters["BDT"],
        ["DNN"]         + global_counters["DNN"],
    ]

    eff_data = [
        ["DeepFlavB",   100*global_counters["DeepFlavB"][2]/global_total_hbb,
                        100*global_counters["DeepFlavB"][1]/global_total_hbb],
        ["DeepFlavCvL", 100*global_counters["DeepFlavCvL"][2]/global_total_hbb,
                        100*global_counters["DeepFlavCvL"][1]/global_total_hbb],
        ["DeepFlavCvB", 100*global_counters["DeepFlavCvB"][2]/global_total_hbb,
                        100*global_counters["DeepFlavCvB"][1]/global_total_hbb],
        ["JetPt",       100*global_counters["JetPt"][2]/global_total_hbb,
                        100*global_counters["JetPt"][1]/global_total_hbb],
        ["BDT",         100*global_counters["BDT"][2]/global_total_hbb,
                        100*global_counters["BDT"][1]/global_total_hbb],
        ["DNN",         100*global_counters["DNN"][2]/global_total_hbb,
                        100*global_counters["DNN"][1]/global_total_hbb],
    ]
    ...
    print(tabulate(df_eff.values.tolist(),
                   headers=df_eff.columns.tolist(),
                   tablefmt="grid"))
